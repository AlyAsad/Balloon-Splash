...
public class EnemyLevel2 : MonoBehaviour
{
    ...
    private void ThrowBalloon()
    {
        ...
        if (choose == 1)
        {
            throwDirection = (PlaceToThrow1.transform.position - throwPoint.position).normalized;
        }
        else
        {
            throwDirection = (PlaceToThrow2.transform.position - throwPoint.position).normalized;
